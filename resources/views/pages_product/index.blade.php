...
@section('content')
    ...
    <div class="m-auto">
        ...
        @endif
        {{-- <a class="btn btn-primary mb-3" href="{{ route('product.create') }}" role="button">Tambah Data</a> --}}
        <br>
            ...
            <tbody>
                @foreach ($product as $item)
                    <tr>
                        ...
                        <td>{{ $item->jenis_produk == 'MB' ? 'Makanan Berat' : 'Laki-laki' }}</td>
                        <td>{{ $item->keluar_produk }}</td>
                        <td>{{ $item->masuk_produk }}</td>
                        <td>{{ $item->nama_pengelola }}</td>
                        <td>{{ $item->nik_pengelola}}</td>
                        <td>{{ $item->alamat_pengelola }}</td>
                        <td>
                            
                            {{-- <div class="d-flex justify-content-center">
                                <a href="{{ route('product.show', ['product' => $item->id_produk]) }}" class="btn btn-info p-1 ml-1">Show</a>
                            <a href="{{ route('product.edit', ['product' => $item->id_produk]) }}" class="btn btn-success p-1 ml-1">Edit</a>
                                <form action="{{ route('product.destroy', ['product'=> $item->id_produk] ) }}" method="POST">
                                    @method('DELETE')
                                    @csrf
